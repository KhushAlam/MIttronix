...
function InvoiceDetails() {
  ...
  const handleStatusUpdate = async (newStatus) => {
    try {
      ...
      await invoiceService.updateInvoice(id, { ...invoice, status: newStatus })
              ...
              <tbody>
                {(invoice.items || []).map((item, index) => (
                  <tr key={item.id || item._id || index}>
                    ...
                    <td className="rate-col">₹{(item.rate || 0).toLocaleString()}</td>
                    <td className="amount-col">₹{(item.amount || 0).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
